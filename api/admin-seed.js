...
const { insertProduct, uploadImage, listUnsoldProducts } = require("./_lib/supabase");

// Popula o banco com as 14 peças de exemplo do protótipo, uma única vez.
// Protegido pela mesma senha do Studio Upload. As imagens são buscadas do
// próprio site publicado (public/assets/gallery), não do disco do servidor.
const SEED = [
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!body.force) {
      const existing = await listUnsoldProducts();
      if (existing.length > 0) {
        return sendJson(res, 409, {
          error: `Já existem ${existing.length} peças no catálogo. Se quiser mesmo assim adicionar as 14 de exemplo de novo, chame com "force": true.`,
        });
      }
    }
    ...
    for (const item of SEED) {
      const imgRes = await fetch(`${siteUrl}/assets/gallery/${item.file}`);
      if (!imgRes.ok) throw new Error(`Não consegui buscar ${item.file} do site (${imgRes.status}).`);
      const buffer = Buffer.from(await imgRes.arrayBuffer());
      const storagePath = `seed-${Date.now()}-${item.file}`;
      ...
      results.push(product.title_en);
    }

    sendJson(res, 200, { ok: true, published: results });
  } catch (err) {
    console.error(err);
    sendJson(res, 500, { error: "Não foi possível popular as peças de exemplo: " + err.message });
  }
};
